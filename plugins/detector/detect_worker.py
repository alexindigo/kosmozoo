...
Long-lived helper spawned by detect_service.py (the HTTP wrapper). Protocol:
JSON Lines over stdin/stdout — each request line is {"id": N, "b64": "<image
bytes>"}; each response is {"id": N, "w": int, "h": int, "faces": [...]} or
{"id": N, "error": "..."}. Prints {"ready": true} once models are loaded.

Relocated unchanged from the outgoing kosmozoo server (tag python-final);
the HTTP wrapper adds the read timeout and stderr surfacing the old
integration lacked.
